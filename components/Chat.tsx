...
export default function Chat() {
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "assistant",
      content:
        "Willkommen! I'm the **Abarxas ISDP Knowledge Gathering Agent**. " +
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-2xl shadow-sm border border-abarxas-100 overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3 border-b border-abarxas-100 bg-gradient-to-r from-abarxas-50 to-white">
        <div>
          <div className="text-xs uppercase tracking-wider text-abarxas-500">
            Active agent
          </div>
          <div className="font-semibold text-abarxas-900">
      ...
      <div className="border-t border-abarxas-100 bg-abarxas-50/50 p-3">
        <div className="flex items-end gap-2 bg-white border border-abarxas-200 rounded-xl p-2 focus-within:ring-2 focus-within:ring-abarxas-500">
          <textarea
            ref={textareaRef}
            value={input}
            ...
            className="flex-1 resize-none bg-transparent outline-none px-2 py-1.5 text-abarxas-900 placeholder:text-abarxas-400 max-h-40"
            disabled={sending}
          />
          <button
            onClick={send}
            disabled={sending || !input.trim()}
            className="flex-shrink-0 bg-abarxas-600 hover:bg-abarxas-700 disabled:opacity-50 text-white rounded-lg p-2 transition"
            aria-label="Send"
          >
            {sending ? (
        ...
        <div className="text-[11px] text-abarxas-400 mt-1.5 px-1">
